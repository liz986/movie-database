...
ttk.Entry(search_frame, textvariable=search_var, width=50).grid(row=0, column=1, padx=5)
...
def on_search_select(event):
    ...
    load_poster(url, poster_label_search)
...
def save_selected_movie():
    selected = tree_search.selection()
    if not selected:
        messagebox.showwarning("No selection", "Select a movie.")
        return
    imdb_id = tree_search.item(selected[0], "values")[0]
    save_movie_to_db(search_results_cache[imdb_id])
    refresh_saved()
    messagebox.showinfo("Saved", "Movie saved successfully!")

ttk.Button(tab_search, text="Save Selected Movie", command=save_selected_movie).pack(pady=10)

# ------------------ SAVED TAB ------------------
def delete_movie(imdb_id):
    if not messagebox.askyesno("Confirm Delete", "Delete this movie?"):
        return
    conn = sqlite3.connect("movies.db")
    cursor = conn.cursor()
...
def delete_selected_movie():
    ...
    if not selected:
        messagebox.showwarning("No selection", "Select a movie to delete.")
        return
...
def on_saved_select(event):
    ...
    row = cursor.fetchone()
    conn.close()
    load_poster(row[0] if row else None, poster_label_saved, is_search=False)
...
def edit_selected_movie():
    ...
    if not selected:
        return
    imdb_id, title, _, _, rating, review = tree_saved.item(selected[0], "values")
...
ttk.Button(tab_saved, text="Delete Selected Movie", command=delete_selected_movie).pack(pady=5)
